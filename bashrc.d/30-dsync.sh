...
dsync() {
  ...
  elif [ -d "$HOME/code-projects/tmux-auto-claude-config/.git" ]; then
    repo="$HOME/code-projects/tmux-auto-claude-config"
  else
    echo "Dotfiles repo not found." >&2
    echo "  Expected: ~/.dotfiles symlink or ~/code-projects/tmux-auto-claude-config/" >&2
    echo "  Re-clone: git clone https://github.com/MartyBonacci/tmux-auto-claude-config.git <path>" >&2
    return 1
  fi
